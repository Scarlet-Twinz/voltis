...
let AnalyticsService = class AnalyticsService {
    ...
    constructor(organizationsRepository, accountsRepository, paymentsRepository, transactionsRepository, ledgerEntriesRepository, riskAssessmentsRepository, reconciliationRunsRepository) {
    ...
    async getOverview(userId, organizationId) {
        const organization = await this.getOrganization(userId, organizationId);
        const [paymentTotals, transactionTotals, accountTotals, riskTotals, reconciliationTotals, ledgerTotals,] = await Promise.all([
            this.paymentsRepository
                .createQueryBuilder('payment')
                .select('COUNT(*)', 'count')
                .addSelect('COALESCE(SUM(CAST(payment.amount AS NUMERIC)), 0)', 'volume')
                .where('payment.organizationId = :organizationId', { organizationId })
                .getRawOne(),
            this.transactionsRepository
                .createQueryBuilder('transaction')
                .select('COUNT(*)', 'count')
                .addSelect('COALESCE(SUM(CAST(transaction.amount AS NUMERIC)), 0)', 'volume')
                .where('transaction.organizationId = :organizationId', { organizationId })
                .getRawOne(),
            this.accountsRepository
                .createQueryBuilder('account')
                .select('COUNT(*)', 'count')
                .addSelect('COALESCE(SUM(CAST(account.balance AS NUMERIC)), 0)', 'balance')
            ...
            this.riskAssessmentsRepository
                ...
                .select('COUNT(*)', 'count')
                .addSelect(`SUM(CASE WHEN risk.decision = :allow THEN 1 ELSE 0 END)`, 'allowed')
                .addSelect(`SUM(CASE WHEN risk.decision = :review THEN 1 ELSE 0 END)`, 'review')
                .addSelect(`SUM(CASE WHEN risk.decision = :block THEN 1 ELSE 0 END)`, 'blocked')
                .addSelect('COALESCE(AVG(risk.score), 0)', 'averageScore')
            ...
            this.ledgerEntriesRepository
                ...
                .innerJoin(Transaction, 'transaction', 'transaction.id = entry.transactionId')
                .select(`COALESCE(SUM(
            CASE
        ...
        const [completedPayments, failedPayments, pendingPayments, processingPayments, completedTransactions, failedTransactions,] = await Promise.all([
        ...
        return {
            ...
            payments: {
                ...
                successRate: paymentCount > 0
                    ? Number(((successfulPaymentCount / paymentCount) *
                        100).toFixed(2))
                    : 0,
            },
            transactions: {
            ...
            ledger: {
                ...
                balanced: String(ledgerTotals?.debits ?? '0') ===
            ...
            risk: {
                ...
                averageScore: Number(Number(riskTotals?.averageScore ?? 0).toFixed(2)),
            },
            reconciliation: {
                total: Number(reconciliationTotals?.count ?? 0),
                completed: Number(reconciliationTotals?.completed ?? 0),
            },
        };
    }
    async getPayments(userId, organizationId) {
        ...
        const byStatus = Object.values(PaymentStatus).reduce((result, status) => {
            result[status] = payments.filter((payment) => payment.status === status).length;
            return result;
        }, {});
        const byMethod = Object.values(PaymentMethod).reduce((result, method) => {
            result[method] = payments.filter((payment) => payment.method === method).length;
            return result;
        }, {});
        const volumeByCurrency = payments.reduce((result, payment) => {
    ...
    async getTransactions(userId, organizationId) {
        await this.getOrganization(userId, organizationId);
        ...
        const byStatus = Object.values(TransactionStatus).reduce((result, status) => {
            result[status] = transactions.filter((transaction) => transaction.status === status).length;
            return result;
        }, {});
        const byType = Object.values(TransactionType).reduce((result, type) => {
            result[type] = transactions.filter((transaction) => transaction.type === type).length;
            return result;
        }, {});
        const volumeByCurrency = transactions.reduce((result, transaction) => {
    ...
    async getRisk(userId, organizationId) {
        await this.getOrganization(userId, organizationId);
        const assessments = await this.riskAssessmentsRepository.find({
            where: {
                organizationId,
            },
            order: {
        ...
        const byDecision = Object.values(RiskDecision).reduce((result, decision) => {
            result[decision] =
                assessments.filter((assessment) => assessment.decision === decision).length;
            return result;
        }, {});
        const averageScore = assessments.length > 0
            ? assessments.reduce((total, assessment) => total + assessment.score, 0) / assessments.length
            : 0;
        return {
            total: assessments.length,
            averageScore: Number(averageScore.toFixed(2)),
            byDecision,
            recent: assessments.map((assessment) => ({
                id: assessment.id,
    ...
    async getAccounts(userId, organizationId) {
        await this.getOrganization(userId, organizationId);
